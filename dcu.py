...
class VideoMonitor(QWidget):
    def __init__(self, video_source):
        ...
        self.original_fps = self.cap.get(cv.CAP_PROP_FPS)
        self.simulated_time = 0
        self.entry_times = {}
        self.exit_times = {}
        self.prev_in_area = {}  # {track_id: bool}
    ...
    def update_frame(self):
        ...
        if ret:
            ...
            self.simulated_time += 1 / self.original_fps
        ...
        if results and results[0].boxes.id is not None:
            ...
            for box, box_C, track_id, name, conf in zip(boxes, boxes_C, track_ids, names, confs):
                ...
                if self.show_bbox:
                    ...
                    # 이전 영역 상태 불러오기 (없으면 False)
                    was_in_A = self.prev_in_area.get(track_id, False)

                    # 평균 속도 업데이트
                    avg_speed_kmph = self.update_average_speed(track_id, transformed_pointA, in_A, was_in_A)

                    # 현재 상태 저장
                    self.prev_in_area[track_id] = in_A
    ...
    def update_average_speed(self, track_id, transformed_point, in_area, was_in_area):
        if in_area and not was_in_area:
            # 진입
            self.entry_times[track_id] = self.simulated_time
            print(f"객체 {track_id}가 A 영역에 진입했습니다. 진입 시간: {self.entry_times[track_id]:.2f}초")

        elif not in_area and was_in_area and track_id in self.entry_times and track_id not in self.exit_times:
            # 이탈
            self.exit_times[track_id] = self.simulated_time
            elapsed = self.exit_times[track_id] - self.entry_times[track_id]
            print(f"객체 {track_id}가 A 영역을 통과했습니다. 경과 시간: {elapsed:.2f}초")

            distance_m = 30  # 예시 기준 A 영역 거리
            avg_speed_mps = distance_m / elapsed
            avg_speed_kmph = avg_speed_mps * 3.6
            print(f"객체 {track_id}의 평균 속도: {avg_speed_kmph:.2f} km/h")
