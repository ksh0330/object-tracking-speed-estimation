...
class VideoMonitor(QWidget):
    def __init__(self, video_source):
        ...
        self.areas = {
            ...
            "Person": [(301, 161), (421, 111), (613, 223), (485, 280)]
        }
        ...
        sourcePerson= np.array([[301, 161], [421, 111], [613, 223], [485, 280]])
        
        target = np.array([[0, 0], [self.T_W, 0], [0, self.T_H], [self.T_W, self.T_H]])
        targetPerson = np.array([[80, 0], [240, 0], [80, 360], [240, 360]])
        ...
        self.transformerPerson = ViewTransformer(source=sourcePerson, target=targetPerson)

        # Initialize variables for drawing
        self.T_FPS = 10
        self.PREDICT = 15
        self.track_history = defaultdict(lambda: deque(maxlen=self.T_FPS))
        self.trackP_history = defaultdict(lambda: deque(maxlen=self.T_FPS))
        ...
        self.trackmP_history = defaultdict(lambda: deque(maxlen=5))
    ...
    def evaluate_collision_risk_P(self):
        if self.current_a_position is not None and self.current_P_position is not None:
            distance = abs(self.current_a_position - self.current_P_position)

            if distance < 50 and self.speed == "F":
                return "위험"
            elif distance < 40 and self.speed == "M":
                return "주의"
            elif distance < 80 and self.speed == "S":
                return "주의"
            else:
                return "안전"
    ...
    def update_display(self, frame, results):
        ...
        if results and results[0].boxes.id is not None:
            ...
            for box, box_C, track_id, name, conf in zip(boxes, boxes_C, track_ids, names, confs):
                ...
                track.append((x, y))
                trackP = self.trackP_history[track_id]
                trackP.append((x, y+h//2))


                if self.show_bbox:
                    if cls_name == "CAR":
                        transformed_pointP = self.transformerPerson.transform_points(np.array([[x, y]]))

                        cv.polylines(frame, [np.array(self.areas['Person'], np.int32)], True, self.BLACK, 1)
                        in_P = cv.pointPolygonTest(np.array(self.areas['Person'], np.int32), (x, y), False) >= 0
                        in_A = cv.pointPolygonTest(np.array(self.areas['Person'], np.int32), (x, y), False) >= 0

                        if in_P:
                            cv.rectangle(frame, (x1, y1), (x2, y2), self.BLUE, 2)
                            cv.putText(frame, f'ID: {int(track_id)} {cls_name}',
                                       (x1, y1 - 10), cv.FONT_HERSHEY_SIMPLEX, 0.5, self.BLUE, 2)


                            points = np.hstack(trackP).astype(np.int32).reshape((-1, 1, 2))
                            cv.polylines(frame, [points], isClosed=False, color=self.BLUE, thickness=3)

                            self.current_a_position = transformed_pointP[0][-1]
                            print(self.current_a_position)

                            self.trackmP_history[track_id].append(transformed_pointP)
                            last_x = int(self.trackmP_history[track_id][-1][-1][0])
                            first_x = int(self.trackmP_history[track_id][0][0][0])

                            if last_x - first_x < 0:
                                print("보행자 접근중")
                                if abs(last_x - first_x) >= 40:
                                    self.speed = "F"
                                elif abs(last_x - first_x) >= 20:
                                    self.speed = "M"
                                else:
                                    self.speed = "S"

                                if in_A:
                                    collision_status = self.evaluate_collision_risk_P()
                    ...
                    if cls_name == "CAR":
                        ...
                        if in_A:
                            ...
                            last_y = int(self.trackmA_history[track_id][-1][-1][1])
                            first_y = int(self.trackmA_history[track_id][0][0][1])
                            if last_y - first_y <= 0:
                                self.A_v_list.clear()
                                self.av = 0
                            else:
                        ...
                        if in_B:
                            ...
                            last_y = int(self.trackmB_history[track_id][-1][-1][-1])
                            first_y = int(self.trackmB_history[track_id][0][0][1])
                            if last_y - first_y <= 0:
                                self.B_v_list.clear()
                                self.bv = 0
                            else:
                        ...
                        if self.current_a_position and self.current_b_position is not None:
                            if self.current_a_position >= 450 or self.current_b_position >= 450:
                                collision_status = self.evaluate_collision_risk()
                                self.collision_label.setText(f"위험도: {collision_status}")

                                if collision_status == "위험":
                                    self.broadcast_message(collision_status + "\n")
                                    print(datetime.datetime.now(), collision_status)
...
if __name__ == "__main__":
    ...
    video_source = r"D:\Straight_Data_0623\Danger\ACBC_F1MD.mp4"
